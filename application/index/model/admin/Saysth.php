...
class saysth extends Model{
	public function get($rn=5,$pn=1){
        ...
        if($pn<=0){$pn=1;}
        $start=($pn-1)*$rn;
        $saysth=Db::query("SELECT `id`,`content`,`time` FROM `saysth` ORDER BY `saysth`.`time` DESC LIMIT $start,$rn");
        return json_encode(['status'=>200,'msg'=>'获取成功','data'=>$saysth,'total'=>Db::query("SELECT count(1) FROM `saysth`")[0]['count(1)']]);
	}
	public function add($content){
        if(empty($content)){return json_encode(['status'=>-200,'msg'=>'参数不全']);}
        $time=date('Y-m-d H:i:s');
        $saysth=Db::name('saysth')->insert(['content'=>$content,'time'=>$time]);
        if($saysth){return json_encode(['status'=>200,'msg'=>'添加成功']);}
        else{return json_encode(['status'=>-200,'msg'=>'添加失败']);}
	}
	public function del($id){
        if(empty($id)&&$id!==0){return json_encode(['status'=>-200,'msg'=>'参数不全']);}
        $saysth=Db::name('saysth')->delete($id);
        if($saysth){return json_encode(['status'=>200,'msg'=>'删除成功']);}
	...
	public function edit($data){
        ...
        $result=Db::table('saysth')->where('id',$data['id'])->update($updateData);
