...
class Shop extends Model{
	...
	public function getGood($id){
        $good=Db::query("SELECT * FROM `shop` WHERE `type`=0 AND `id`=$id");
        return json_encode(['status'=>200,'msg'=>'获取成功~','data'=>$good[0]]);
	}
	public function buyGoods($GoodIdAndHowManyArr){
        // $GoodIdAndHowManyArr=[0=>['id'=>0,'value'=>1]]
        $id=session('uid');
        if(empty($GoodIdAndHowManyArr)||empty($id)){return json_encode(['status'=>-200,'msg'=>'参数不全~']);}
        $all=0;
        $str='';
        ...
        if($all>$point){return json_encode(['status'=>-200,'msg'=>'抱歉哦~您的积分不够呢~']);}
    ...
    public function useGood($id,$remark=''){
        ...
        //发送mail
        if(sendUseGoodMail(null,2,$remark)===false){return json_encode(['status'=>-200,'msg'=>'使用失败!']);}
        //减去他的个数(加一个-1的)
    ...
    public function getBag(){
        if(!session('uid')){return json_encode(['status'=>-200,'msg'=>'获取失败~']);}
        $id=session('uid');
        ...
        sort($list); 
        return json_encode(['sttaus'=>200,'msg'=>'获取成功~','data'=>$list]);
    }
}